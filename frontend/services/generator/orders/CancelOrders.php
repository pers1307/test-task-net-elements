...
class CancelOrders
{
    ...
    {
        $count = Order::find()
            ->where(['lost' => 0, 'cancel' => 0])
            ->count();

        $percent = mt_rand(2, 4);

        return ceil($count * $percent / 100);
    }
    ...
    public function add()
    {
        ...
        $divideProductsOnOrdersService->sliceNewProductOnCancelOrders();
